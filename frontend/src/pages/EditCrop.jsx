...
function EditCrop() {
  ...
  useEffect(() => {
    const fetchCrop = async () => {
      try {
        ...
        setError(t('edit_crop.load_fail'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (e) => {
    ...
    if (!user || !user.token) {
      setError(t('edit_crop.login_required'));
      return;
    }
    ...
    try {
      ...
      await api.put(`/api/crops/${id}`, cropData, config);
      setMessage(t('edit_crop.success_msg'));
      ...
      setError(
        err.response && err.response.data.message
          ...
          : t('edit_crop.fail_msg')
      );
    }
  };

  if (loading) return <div style={{ maxWidth: '800px', margin: '40px auto' }}>{t('edit_crop.loading')}</div>;

  return (
    <div style={{ maxWidth: '800px', margin: '40px auto' }}>
      <div style={{ marginBottom: '30px' }}>
        <h1 style={{ margin: '0 0 8px 0', fontSize: '2.2rem' }}>{t('edit_crop.title')}</h1>
        <p style={{ margin: 0, color: 'var(--neutral-500)', fontSize: '1.05rem' }}>{t('edit_crop.subtitle')}</p>
      </div>
        ...
        <form onSubmit={onSubmit} className="card-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          ...
          <div style={{ gridColumn: '1 / -1', marginTop: '20px' }}>
            ...
            <button 
              ...
              style={{ fontSize: '1.1rem', padding: '14px', marginTop: '10px' }}
            >
              {t('edit_crop.cancel')}
            </button>
          </div>
